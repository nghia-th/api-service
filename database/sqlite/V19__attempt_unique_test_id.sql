...
-- Step 2: now that every test_id maps to at most 1 row, enforce it going forward. SQLite has no
-- ALTER TABLE ... ADD CONSTRAINT - a UNIQUE index is the standard equivalent and enforces the same
-- guarantee.
CREATE UNIQUE INDEX uq_attempt_test_id ON attempt (test_id);
