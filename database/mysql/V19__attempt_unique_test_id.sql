...
-- created row - and delete the rest (their attempt_answer children first, to satisfy the FK). The
-- extra "SELECT id FROM (...) dup_ids" wrapping is required by MySQL, which otherwise refuses a
-- DELETE whose subquery reads from the same table being deleted from.
